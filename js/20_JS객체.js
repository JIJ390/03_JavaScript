/* 
  ...
  - (중요!) K(key) 는 무조건 string(문자형)
    -> "Key" 또는 'key' 또는 key(따옴표 안붙여도 자의적으로 문자형으로 해석)
...
btn1.addEventListener('click', () => {
  ...
  // 3. 특정 key 의 value 수정하기
  // 이미 존재하는 key 에 새로운 값 대입 ==> 값 덮어쓰기
  product.price = 2000;
  console.log(product);
  ...
  // - delete 객체명.key -> 객체에서 K:V 삭제 (value 도 함께)
...
btn2.addEventListener('click', () => {
  ...
  person.introduce = function() {
    ...
    // name 과 age 가 전연변수인지 지역변수인지 인식이 안됨
    // gender 는 지역변수로 존재
    /* 
      this 참조 변수  
      -> 작성된 위치마다 의미가 다름
      ...
      1) 객체 내 함수(메서드) 에서 this == 같은 객체(여기, 이 객체) 
        이 경우, this == person
        * 이 케이스를 가장 많이 사용, 2, 3 번은 다른 방법을 주로 사용함

      2) <button onclick='testFn(this)'>테스트</button>
        이 경우, this == 이벤트가 발생한 요소(클릭된 button)

      3) 화살표 함수 내에서 this == Window 객체
        (화살표 함수에서는 this 를 못쓴다고 생각!!)
    */
  }
...
/* 입력 값을 이용해 Student 객체를 생성해 배열에 저장 */
...
addStudent.addEventListener('click', () => {
  // input 에 작성된 값을 이용해 학생 객체 생성
  const std = new Student(inputGrade.value,
                          inputBan.value, 
                          inputNumber.value, 
                          inputName.value);
  // 생성된 Student 객체를 studentList 에 저장
...
printStudent.addEventListener('click', () => {
  ...
  // studentList 에 존재하는 모든 학생 객체 접근하기
...
checkJSON.addEventListener('click', () => {
  ...
  // 기상청 서버에 7/ 12 오전 10 전국 미세먼지 농도를 JSON 으로 받아오기
