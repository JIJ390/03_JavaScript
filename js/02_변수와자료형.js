...
// =(등호) : 대입 연산자
// A = B -> B 를 A 에 대입 ( 오른쪽 값을 왼쪽 변수에 대입 )
...
function varCheck() {
  ...
  // var 를 이용할 경우
  // 변수명이 중복되면 이전 값을 새로운 값으로 덮어쓰기 함
  // 문제점 : 이전에 대입한 값을 이용할 수 없다
  //          -> 나중에 이전 값을 사용하고 싶은데 새로운 값을 대입해 지워버리는 경우가 생김
}
...
function constCheck() {
  ...
  // -> 상수 변수에 값이 할당됨
  // at constCheck (02_변수와자료형.js:86:16)

  // 해결 방법 : const 를 let 으로 변경
...
//num3 변수에 저장된 값이 300 과 같다면 {} 내부 코드 수행
  if(num3 == 300) {
...
function scopeTest2() {
  ...
  if(1 == 1) {
    ...
    //함수 지역 변수 temp 와 같은 이름의 if 지역 변수 temp를 선언
    const temp = 456;
  ...
  //if 문 지역 변수 local1을 if 문 밖에서 호출하면 '정의 되지 않음' 오류 발생
}
...
function typeCheck() {
  //함수 정의
  /* tpeof 연산자 : 변수/ 값의 자료형을 확인해주는 연산자 */
  ...
  */
  ...
  console.log( sumFn(300, 400), typeof sumFn(300, 400) );
}
